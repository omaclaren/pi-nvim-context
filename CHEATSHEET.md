...
Mappings below use `<leader>` notation. The plugin does not set `mapleader`; Vim and Neovim use backslash (`\`) by default. If your configuration sets `<leader>` to Space, for example, `<leader>ps` means `Space p s`. Pi does not require Copilot or another inline completion plugin. This example leaves automatic Insert-mode completion to Copilot while Pi handles deliberate requests.

## Which tool should I use?

| Goal | Use |
|---|---|
| Ask Pi a question about editor content | Use `<leader>pf/pl/ps/pd/pb`, then write the question in Pi |
| Get a deliberate completion at one cursor position | `<leader>pc` |
| Insert something specific at the cursor | `<leader>pg`, then enter the instruction |
| Rewrite selected text according to an instruction | Visual `<leader>pr` |
...
| Mode | Mapping | Action |
|---|---|---|
| Normal | `<leader>pf` | Add the current file reference |
| Normal | `<leader>pl` | Add file, cursor location, and current in-memory line |
...
| Mode | Mapping | Action |
|---|---|---|
| Normal | `<leader>pc` | Request a completion **after** the cursor character |
| Normal | `<leader>pg` | Enter an instruction and request an insertion there |
| Visual | `<leader>pr` | Enter an instruction and request a selection rewrite |
| Normal | `Tab` | Accept while a Pi result is visible, when temporary Tab acceptance is enabled and available |
| Normal | `<leader>pa` | Explicit accept mapping; always available by default |
| Normal | `<leader>pn` | Generate a materially different result |
| Normal | `<leader>pv` | Focus the full preview; scroll normally and press `q` to return |
| Normal | `<leader>px` | Cancel an active request or dismiss its result |
...
5. Press Normal `Tab` to accept when available, or use `<leader>pa` / `<leader>pn` / `<leader>px`.
...
5. Press Normal `Tab` when available, or `<leader>pa`, to accept. Use `<leader>pv` first if you need to focus and scroll the preview.
...
4. Review the wrapping diff preview; use `<leader>pv` to focus and scroll it.
5. Press Normal `Tab` when available, or use `<leader>pa` / `<leader>pn` / `<leader>px`.
...
| Normal | `<leader>ct` | Toggle Copilot globally |
...
| Mode | Mapping | Action |
|---|---|---|
| Normal | `<leader>y` followed by a motion | Yank that motion to the system clipboard |
| Normal | `<leader>yy` | Yank the current line to the system clipboard |
| Visual | `<leader>y` | Copy the selection to the system clipboard |
...
4. In Neovim, run `:PiContextStatus` (default: `<leader>pi`).
5. Run `:PiContextPick` (default: `<leader>pp`) to inspect and explicitly choose among discovered Pi bridges.
